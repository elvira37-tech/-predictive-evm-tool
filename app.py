...
if assets:
    ...
    if st.button("📈 RUN PREDICTIVE ANALYSIS", type="primary"):
        ...
        # Map Baseline (PV)
        for i in range(6):
            p_start, p_end = days_planned_cum[i], days_planned_cum[i+1]
            p_budget = total_cost * c_weights[i]
            if p_end > p_start:
                daily_pv_rate[p_start:p_end] = p_budget / (p_end - p_start)
                
        # Map Reality (AC and EV)
        for i in range(6):
            r_start, r_end = reality_days[i], reality_days[i+1]
            p_budget = total_cost * c_weights[i]
            r_cost = actual_data[i]['cost'] if i in actual_data else pred_costs[i]
            if r_end > r_start:
